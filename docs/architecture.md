...
- DeepSeek 分批泛读全部候选、保留全局前 40 后复排；只有最终前 10 尝试 Analyst 与 Verifier，失败不从第 11 条补位。材料门合格项进入“精读”，其余入围项成为不写分析缓存的“泛读”线索。主题词只看标题和速读句；精读与今日速读按首次出现主题稳定分组，同主题相邻且组内保留 Scout 顺序。
- 前一天已经发布过的事件，第二天入选权重降低。泛读一级页按主题聚类且只显示标题，二级详情在同一 HTML 内显示摘要、要点和来源。Git 页只展示 GitHub 今日最热和本周增长最快，并逐仓库读取 README、清单与源码入口，生成具体使用场景并标明当前总星标；日/周增量只在副标题显示。
